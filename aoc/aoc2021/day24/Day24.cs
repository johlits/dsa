...
public class Day24
{
    private static long FindModelNumberParallel(Machine machine,
    ...
    private static long? FindModelNumber(Machine machine,
                          IEnumerable<long> digitInputs,
                          int digits = 14,
                          long value = 0,
    ...
    private static IEnumerable<T> EnumSingle<T>(T value)
    {
        yield return value;
    }

    private static Machine Execute(Machine machine, IEnumerable<long> input)
    {
    ...
    private static bool Step(Instruction instruction, Func<long?> input, Func<Parameter, long> read, Action<VariableParameter, long> write)
    {
        switch (instruction)
        {
    ...
    private static long Calculate(Operation operation, long arg1, long arg2)
      => operation switch
      {
